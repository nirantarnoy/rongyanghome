...
try {
    ...
    if ($action == 'save') {
        ...
        if (mysqli_query($conn, $sql)) {
            ...
            // logAction is a more generic function if logQuotation is specific
            if (function_exists('logAction')) {
                logAction($conn, ($id ? "แก้ไข" : "สร้าง") . "ใบสั่งซื้อ: $doc_number", $id ? 'update' : 'create', $saved_id);
            }
        ...
        $sql = "SELECT * FROM purchase_orders WHERE company_id = $company_id " . ($search ? "AND (doc_number LIKE '%$search%' OR vendor_name LIKE '%$search%')" : "") . " ORDER BY created_at DESC";
        ...
        $id = (int)($_GET['id'] ?? 0);
        $sql = "SELECT * FROM purchase_orders WHERE id = $id AND company_id = $company_id";
        $result = mysqli_query($conn, $sql);
        ...
        $id = (int)($_POST['id'] ?? 0);
        $sql = "DELETE FROM purchase_orders WHERE id = $id AND company_id = $company_id";
        if (mysqli_query($conn, $sql)) {
            $response = ['status' => 'success', 'message' => 'ลบใบสั่งซื้อเรียบร้อยแล้ว'];
        } else {
            throw new Exception(mysqli_error($conn));
        }
    } else {
