        ...
        <div class="item-row border border-gray-200 rounded-lg p-4 bg-gray-50" data-item="${itemCount}">
            <div class="grid grid-cols-12 gap-3">
                ...
                <div class="col-span-3 md:col-span-1">
                    ...
                    <input type="number" class="item-qty w-full px-3 py-2 border border-gray-200 rounded-xl text-sm text-center" value="${data?.qty || 1}" step="0.01" onchange="calculateTotal()">
                </div>
                <div class="col-span-3 md:col-span-1">
                ...
                <div class="col-span-3 md:col-span-2">
                    ...
                    <input type="number" class="item-price w-full px-3 py-2 border border-gray-200 rounded-xl text-sm text-right" value="${data?.price || 0}" step="0.01" onchange="calculateTotal()">
                </div>
        ...
        itemsHTML += `
            <tr style="border-bottom: 1px solid #000;">
                ...
                <td style="padding: 5px; text-align: center; border-right: 1px solid #000;">
                    ${image ? `<img src="${image}" style="max-height: 50px; border: 1px solid #eee;">` : ''}
                </td>
            ...
            <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-top: 20px;">
                ...
                <div style="flex: 1; display: flex; justify-content: space-around; text-align: center;">
                    <div style="width: 200px;">
                        <p style="margin-bottom: 60px; font-size: 11px;">ผู้สั่งขาย</p>
...
function exportPDF() {
    const id = $('#so_id').val() || $('#sales_order_id').val();
    if (id) {
        window.open('export_pdf.php?type=sales_order&id=' + id, '_blank');
    } else {
        Swal.fire({
...
function saveSOAndExportPDF() {
    ...
    const data = {
        action: 'save',
        id: $('#so_id').val() || $('#sales_order_id').val(),
        doc_number: docNumber,
        doc_date: docDate,
        customer_name: customerName,
        customer_address: $('#customer_address').val(),
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (res.status === 'success') {
                $('#so_id').val(res.id);
                Swal.close();
                window.open('export_pdf.php?type=sales_order&id=' + res.id, '_blank');
            } else {
