<div class="bg-white rounded-2xl shadow-sm p-6 mb-6">
    <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
        <h2 class="text-xl font-bold text-gray-800">รายการใบรับสินค้า</h2>
        <div class="flex gap-3 w-full md:w-auto">
            <div class="relative flex-1 md:w-64">
                <input type="text" id="searchInputGR" placeholder="ค้นหา..." 
            ...
            <a href="goods_receipt_form.php" class="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg transition-all flex items-center gap-2 whitespace-nowrap">
        ...
        <table class="w-full">
            <thead class="bg-gray-50 border-b">
                <tr>
                    ...
                    <th class="px-6 py-4 text-left text-sm font-semibold text-gray-600">ผู้ขาย/ผู้ส่ง</th>
            ...
            <tbody id="grList" class="divide-y divide-gray-100">
                <tr>
...
function loadGRs() {
    const search = $('#searchInputGR').val();
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.status === 'success') {
                renderGRList(response.data);
            }
        }
    });
}

function renderGRList(data) {
    let html = '';
            ...
            html += `
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-6 py-4 text-center space-x-2">
                        ...
                        <button onclick="deleteGR(${item.id})" class="inline-flex items-center px-3 py-1.5 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition-all text-sm font-medium">
                            🗑️ ลบ
                        </button>
                    </td>
                </tr>
...
function deleteGR(id) {
    Swal.fire({
        title: 'ยืนยันการลบ?',
        text: 'คุณต้องการลบใบรับสินค้านี้ใช่หรือไม่?',
        icon: 'warning',
