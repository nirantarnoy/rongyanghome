            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <div class="flex gap-2">
                        ...
                        <button type="button" onclick="openSignaturePad(1)" class="bg-purple-100 text-purple-700 px-3 py-1 rounded-lg hover:bg-purple-200 transition-all text-xs flex items-center gap-1 h-fit">
                ...
                <div>
                    ...
                    <div class="flex gap-2">
                        ...
                        <button type="button" onclick="openSignaturePad(2)" class="bg-purple-100 text-purple-700 px-3 py-1 rounded-lg hover:bg-purple-200 transition-all text-xs flex items-center gap-1 h-fit">
                            <i class="fas fa-pen-nib"></i> เซ็นชื่อ
                        </button>
...
function generatePreview(showModal = true) {
    ...
    JSON.parse(JSON.stringify(eval($('.item-row').map(function() {
        return {
            ...
            image: $(this).find('img[id^="item_img_"]').attr('src') || ''
        };
    }).get()))).forEach(item => {
...
function exportPDF() {
    const id = $('#receipt_id').val();
    if (id) {
        window.open('export_pdf.php?type=receipt&id=' + id, '_blank');
    } else {
        Swal.fire({
            ...
            if (res.isConfirmed) {
                saveReceiptAndExportPDF();
            }
        });
    }
}

function saveReceiptAndExportPDF() {
    const docNumber = $('#doc_number').val().trim();
    const docDate = $('#doc_date').val();
    const customerName = $('#customer_name').val().trim();
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (res.status === 'success') {
                ...
                Swal.close();
                window.open('export_pdf.php?type=receipt&id=' + res.id, '_blank');
            } else {
