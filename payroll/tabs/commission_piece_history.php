<div class="grid grid-cols-1 gap-6">
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div class="p-6 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-50/50">
            ...
            <button onclick="switchTab('commission_piece_calc')" class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl text-xs transition-all shadow-md shadow-blue-500/10 gap-1.5">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody id="commissionPieceHistoryTableBody" class="divide-y divide-slate-100">
                    <!-- Dynamic -->
                </tbody>
            </table>
        </div>
...
<script>
    function loadCommissionPieceHistory() {
        $.ajax({
            url: 'payroll_action.php',
            type: 'GET',
            data: { action: 'list_commission_transactions', commission_type: 'piece' },
            success: function(data) {
                    ...
                    data.forEach(function(row) {
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-xs text-right space-x-2">
                                <button onclick="editPieceCommissionTransaction(${row.id})" class="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg transition-all" title="แก้ไข">
                                    <i class="fa-solid fa-pen-to-square"></i>
                                </button>
                                <button onclick="deletePieceCommissionTransaction(${row.id})" class="p-1.5 text-rose-600 hover:bg-rose-50 rounded-lg transition-all" title="ลบ">
                ...
                $('#commissionPieceHistoryTableBody').html(html);
            }
        });
    }

    function editPieceCommissionTransaction(id) {
        $.ajax({
            url: 'payroll_action.php',
            type: 'GET',
            ...
            success: function(res) {
                if (res.status === 'success') {
                    ...
                    switchTab('commission_piece_calc');
                    
                    activeEditPieceCommId = res.commission.id;
                    $('#commission_transaction_date').val(res.commission.transaction_date);
                    $('#commissionItemsContainer').html('');
                    
                    res.items.forEach(function(item) {
                        addCommissionPieceItemRow(item);
                    });
    ...
    function deletePieceCommissionTransaction(id) {
        Swal.fire({
            title: 'ยืนยันการลบ?',
