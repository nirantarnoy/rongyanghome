...
// Self-Migration: Create all required tables
    ...
    // Create project_assigned_subcontractors table
    mysqli_query($conn, "CREATE TABLE IF NOT EXISTS project_assigned_subcontractors (
        id INT AUTO_INCREMENT PRIMARY KEY,
        company_id INT NOT NULL,
        project_id INT NOT NULL,
    ...
    // Seed sample subcontractors ONLY if subcontractors table is empty
    $check_subs = mysqli_query($conn, "SELECT id FROM subcontractors LIMIT 1");
    if (mysqli_num_rows($check_subs) == 0) {
        $subcontractors = [
